<template>
  <div class="OutStationInside">
    <van-cell-group>
      ...
      <van-radio-group v-model="sex" readonly style="float:right;">
      ...
      <div class="InputItem">
        <span class="LabelBlock" style="width: 100px;">旗内目的地址</span>
        <div class="InputCon">
          <span>准格尔旗</span>
  ...
  methods: {
    ...
    chooseQ_M () {
      ...
      send({
        ...
        this.QcolumnsID_M = _res.data.qulist_M.map(item => {
          return item.id
        })
      }).catch((res) => {
        // Toast.fail('Interface error！')
    ...
    submitSign () {
      // if (this.car === '请选择' || !this.name || !this.sex || !this.ID || !this.phone || !this.carNo || !this.Tt || !this.Ct || !this.St_M || !this.Qt_M || !this.Zt_M || !this.addr_D || !this.addr_M || !this.ifContact || !this.symptom) {
      //   Toast({
      //     duration: 1500,
      //     forbidClick: true,
      //     message: '请将信息填写完整!'
      ...
      // }
      send({
        name: '/signInInsert?ischangzhu=0&userid=' + this.openid + '&bustype=' + this.car + '&username=' + this.name + '&fsex=' + this.sex + '&idnumber=' + this.ID + '&fmobile=' + this.phone + '&iswuhan=' + (this.ifContact === '有' ? 1 : 0) + '&areaid=' + this.AreaId  + '&ftype=' + this.ftype + '&mdd=' + this.Tt + this.Ct + this.addr_D + '&cfd=' + this.St_M + this.Zt_M  + this.Qt_M + this.addr_M + '&bus_box=' + this.carNo + '&shenti_type=' + this.symptom + '&thisftemp=' + this.temperature + '&code_type=' + this.inOrOutP,
        method: 'POST',
        data: {
        }
      }).then(_res => {
    ...
    submitInsert () {
      ...
      // }
      send({
        name: '/userinsert?ischangzhu=0&open_id=' + this.openid + '&bustype=' + this.car + '&username=' + this.name + '&fsex=' + this.sex + '&idnumber=' + this.ID + '&fmobile=' + this.phone + '&iswuhan=' + (this.ifContact === '有' ? 1 : 0) + '&areaid=' + this.AreaId  + '&ftype=' + this.ftype + '&mdd=' + this.Tt + this.Ct + this.addr_D + '&cfd=' + this.St_M + this.Zt_M  + this.Qt_M + this.addr_M + '&bus_box=' + this.carNo + '&shenti_type=' + this.symptom + '&thisftemp=' + this.temperature + '&code_type=' + this.inOrOutP,
        method: 'POST',
        data: {
        }
      }).then(_res => {
...
.OutStationInside .van-field__control{
